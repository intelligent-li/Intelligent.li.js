...
(function(root) {
  ...
  function Api(host) {
    ...
    this.subscribe = function(id, start) {
      ...
      Meteor.call("subscribe", feed.id, this._clientId, feed.start, function(error) {
    ...
    this.downloader = function(ids, start, end, notify) {
      logger.info("HELLO : " + ids + ' : ' + start + ' ' + end);
      var fromDT = Date.parse(start) / 1000;
      var endDT = Date.parse(end) / 1000;

      Meteor.call("downloader", ids, fromDT, endDT, function(error, result) {
        if (typeof error != 'undefined') {
