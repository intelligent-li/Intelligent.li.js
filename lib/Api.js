...
(function(root) {
  ...
  var ili = root.ili,
    ...
    querystring = ili.importPackage(root, 'querystring', 'querystring'),
  ...
  function Api(host) {
    ...
    // Loads the blob resource, notifying when done with notify(success, blob);
    this.loadBlob = function(collection, sensor, index, alignment, notify) {
      var resource = "sources/" + collection + "/" + sensor + "/blobs/" + alignment + "/" + index;
      var b = {};
    ...
    this.loadBlobs = function(id, st, end, ext, name, dataHandler, notify) {
      var resource = "sources/" + id + 
        "/blobs?start=" + st +
        "&end=" + end + 
        (name == "" ? "" : "&name=" + querystring.escape(name)) +
        (ext == "" ? "" : "&extension=" + querystring.escape(ext));

      var blobParser = function(map, data) { }

      this.loadResource(resource, {}, function(s){ notify(s); }, function() {}, true, dataHandler);
    }
    ...
    this.loadResource = function(
        resource, map, notify, parser, binary, clientDataHandler) {
      try {
          if (!this.agent) {
            this.agent =  new KeepAliveAgent.Secure({
              ...
              ca  : this.certs.ca });
            //Hack to work around slow performance 
            //when there are many outstanding connections.
            //I need to investigate this in more depth to implement 
            //a root cause fix.
            this.agent.maxSockets = 1024;
          }
          ...
          var dataHandler = clientDataHandler || function(d) {
            data += d; 
          }
          ...
          var responseHandler = function(res) {
              ...
              if (res.statusCode != 200){
                 logger.error(res.statusCode + " for " + uri);
                 //consume the data so that the socket gets released.
                 res.on('data', function() {});
                 notify && notify(false);
                 return;
              }
              res.on('data', function(d) { dataHandler(d); });
