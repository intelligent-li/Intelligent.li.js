...
(function(root) {
  ...
  function MeteorConnector() {
    ...
    this._getTempFile = function(fb, ext) {
      var fname = fb + ext;
    ...
    this.blobDownloader = function(id, start, end, ext, fb) {
      var fut = new Future();
      api.loadBlobs(id, start, end, ext, function(success, msg) {
        var fname = _self._getTempFile(fb + "-" + start + "-" + end, '.zip');
        fs.writeFile(fname, msg['data'], 'binary', function(err) {
          if (err) throw err;
    ...
    this.downloader = function(ids, start, end, fb) {
      ...
      var gencsv = function(json) {
        ...
        json2csv({data: json, fields: ids}, function(err, csv) {
          ...
          var fname = _self._getTempFile(fb + Date.now(), '.csv');
          fs.writeFile(fname, csv, function(err) {
            if (err) throw err;
            logger.debug('file saved ' + fname);
            logger.info("finished generating CSV in file " + fname);
            fut['return'](fname);
          });
